 ...
 * 域名管理控制器
 *
 * @extends AbstractCrudController<DnsDomain>
 */
...
final class DnsDomainCrudController extends AbstractCrudController
{
    ...
    public function configureActions(Actions $actions): Actions
    {
        $syncRecordsAction = Action::new('syncRecords', '同步远程到本地')
            ...
            ->displayIf(static function (DnsDomain $domain): bool {
                ...
                return true === $domain->isValid() && null !== $domain->getZoneId();
            })
        ;
    ...
    public function configureFields(string $pageName): iterable
    {
        ...
        // Account ID 是通过关联获取的，只在显示页面显示，不在表单中编辑
    ...
    public function syncRecords(AdminContext $context): Response
    {
        ...
        assert($domain instanceof DnsDomain);

        // 验证域名是否有效
        if (true !== $domain->isValid()) {
        ...
        try {
            ...
            if (null === $domainId) {
                throw new DnsDomainException('域名ID不能为空');
            }
